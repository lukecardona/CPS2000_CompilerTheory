...
vector<string> split(const string &s, char delimiter) { //Custom Function
...
vector<string> splitLine(const string& s, const string& delimiters) {
    ...
    while (getline(tokenStream, token)) {
        ...
        while (end != string::npos) {
            ...
            if (token[end] != ' ') { // Exclude space from delimiter list
                tokens.push_back(token.substr(end, 1)); // Add the delimiter as a separate token
            }
            start = end + 1;
...
vector<string> loadProgramFile(const string& path){
    vector<string> tokens;
    string line;
...
string vectorToString(const vector<string>& programTokens) {
    ostringstream oss;
...
void Lexer::LexerPassFile(const string& path) {
    ...
    string programString = vectorToString(programVector);
...
int main(){
    ...
    lex.getProgramVector();
    lex.getRevisedTokens();
    return 0;
};